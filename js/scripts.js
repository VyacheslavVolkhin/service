$(document).ready(function(){
	...
	$('.js-tabs-nav li a, .js-tabs-nav .js-tab-button').on('click', function() {
		if ($(this).hasClass('active')) {
    ...
    //frm counter
    $('.js-counter-cat .js-button-counter-minus').on('click', function () {
        var cnt = $(this).parents('.js-counter-cat').find('.js-input-counter').val();
        cnt = parseInt(cnt);
        if (cnt > 1) {
            $(this).parents('.js-counter-cat').find('.js-input-counter').val(cnt - 1);
        }
        if (cnt === 1) {
            $(this).parents('.cat-action-wrap').find('.btn.active').removeClass('active');
        }
        return false;
    })
    $('.js-counter-cat .js-button-counter-plus').on('click', function () {
        var cnt = $(this).parents('.js-counter-cat').find('.js-input-counter').val();
        $(this).parents('.js-counter-cat').find('.js-input-counter').val(cnt - 1 + 2);
        return false;
    })
    ...
    //page-actions-slider-box
    if (!!$('.page-actions-slider-box').offset()) {
        $('.page-actions-slider-box .slider').slick({
            dots: true,
            slidesToShow: 1,
            variableWidth: false,
            infinite: true,
